...
public class BlogPostDAO {
	...
	public void likePost(final String permalink, final int ordinal) {
		// System.out.println(ordinal+"   "+permalink);
		...
		// getting old query
		DBObject query1 = new BasicDBObject("permalink", permalink);
		DBObject doc1;
		ArrayList<DBObject> comment1;
		DBObject comments1 = postsCollection.findOne(query1);
		comment1 = (ArrayList<DBObject>) comments1.get("comments");
		doc1 = comment1.get(ordinal);
		/*
		 * updating with new doc (unable to get old result after doc.put(.....)
		 * so created a new query)
		 * http://docs.mongodb.org/manual/core/update/#update
		 * -a-document-element-without-specifying-its-position
		 */
		int num_likes = 0;
		DBObject query = new BasicDBObject("permalink", permalink);
		DBObject doc;
		ArrayList<DBObject> comment;
		DBObject comments = postsCollection.findOne(query);
		comment = (ArrayList<DBObject>) comments.get("comments");
		doc = comment.get(ordinal);
		try {
			num_likes = (Integer) doc.get("num_likes");
		} catch (NullPointerException e) {
			System.out.println("caught");
		}
		doc.put("num_likes", ++num_likes);

		postsCollection.update(new BasicDBObject("permalink", permalink)
				.append("comments", doc1), new BasicDBObject("$set",
				new BasicDBObject("comments.$", doc)), true, false);
	}
}
